...
export class MuseumScene extends Phaser.Scene {
  ...
  create(){ this.cameras.main.setBackgroundColor('#17333a'); addBackButton(this); addTitle(this,'博物館・図鑑','素材9・魚12・イベントカード6');
    const s=loadSave(); const foundFish=Object.keys(s.fishRecords).length; const foundItems=s.discoveredItems.length;
    ...
    fishCatalog.forEach((fish,i)=>{const size=s.fishRecords[fish];this.add.text(25+(i%2)*180,350+Math.floor(i/2)*31,size?`${'★'.repeat(fishRarity[fish])} ${fish} ${size}cm`:'？？？ 未発見',{fontSize:'12px',color:size?'#dff5fb':'#65767a'});});
