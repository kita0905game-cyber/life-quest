...
export class HouseScene extends Phaser.Scene {
  ...
  create() {
    ...
    const status = this.add.text(195, 410, `採掘力 ${loadSave().energy} / 12`, { fontSize: '17px', color: '#f2e4c4' }).setOrigin(0.5);
    ...
    rest.on('pointerup', () => {
      updateSave((save) => ({ ...save, energy: 12 }));
      status.setText('採掘力 12 / 12　回復した！');
