  ...
  return (
    <main className="app-shell">
      <header className="app-header">
        <div>
          <span className="eyebrow">PERSONAL FRONTIER RPG · V0.3</span>
          <h1>LIFE QUEST</h1>
        </div>
      ...
      <section className="status-panel" aria-label="冒険者ステータス">
        ...
        <div className="resource-row"><span>🪙 {save.gold}G</span><span>⚡ {save.energy}</span><span>🪱 {save.bait}</span><span>🎫 {save.explorationTickets}</span><span>🎁 {save.chests}</span></div>
      </section>
      <section className="game-frame">
        <LifeQuestGame />
      </section>
      <p className="footnote">V0.3 — 12種の魚・素材図鑑・宝箱・施設強化・放置探索・Chapter / 自動セーブ</p>
    </main>
  );
}
